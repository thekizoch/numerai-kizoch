...
# Predict on all v2 BQ models func
def predict_bq_v2():
    # df from bq of model registry
    bqclient = bigquery.Client()
    df_v2_bq_models = bqclient.query("SELECT * FROM \
    `numerai-kizoch.model_registry.v2_bq_models`").to_dataframe()
    # Predict on all v2 BQ models
    for index, row in df_v2_bq_models.iterrows():
    ...
    FROM
      ML.PREDICT (MODEL `{}`,
        ...
        FROM
          `numerai-kizoch.big_data.tournament_data`
        )
      )
    ORDER BY row ASC
...
def submit_bq_submissions_v2():
    ...
    for table_id in list_table_ids:
        if 'v2_bq_models' == table_id:
            df_bq_v2_models = bqclient.query('SELECT * FROM model_registry.{}'.format(table_id)).to_dataframe()

    # write all submission.csv's to GCS, and download to local /tmp
    for index, row in df_bq_v2_models.iterrows():
        MODEL_NAME = row[0]
